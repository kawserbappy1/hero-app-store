...
const AppsDetails = () => {
  ...
  const { image, companyName, size, downloads, ratingAvg, reviews, description, ratings } = findAppdataAppDetails;

  const handleAddtoInstallationPage = () => {
    const existingList = JSON.parse(localStorage.getItem("applist")) || [];
    const isDuplicate = existingList.some((p) => p.id === findAppdataAppDetails.id);
    if (isDuplicate) {
      toast.error("This App already added to Installation page", { theme: "light" });
      setInstalled(true);
      return;
    }
    const updatedList = [...existingList, findAppdataAppDetails];
    localStorage.setItem("applist", JSON.stringify(updatedList));
    toast.success("This App successfully added to Installation page", { theme: "colored" });
    setInstalled(true);
  };
    ...
    <div className="max-w-[1440px] mx-auto px-2 py-10 md:px-4 lg:px-6 xl:px-8">
      <div className="flex flex-col md:flex-row gap-4 lg:gap-10 pb-2">
        <div className="max-w-md mx-auto">
          <img src={image} alt={companyName} className="h-full" />
        </div>
        <div className="flex-1">
          <h3 className="text-2xl md:text-xl font-bold">{findAppdataAppDetails.title}</h3>
            ...
            <button
              ...
              className={`px-5 py-3 rounded-md mt-7 transition-colors duration-200 ${
                installed
                  ...
                  : "bg-[#00D390] text-white hover:bg-[#00b87d] cursor-pointer"
              }`}
            >
      ...
      {ratings && (
        <div className="w-full h-[250px] md:h-[300px] lg:h-[350px] p-2 md:p-5 rounded-md mb-10">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart
              layout="vertical"
              data={[...ratings].sort((a, b) => b.name.split(" ")[0] - a.name.split(" ")[0])}
              margin={{
                top: 20,
                right: 40,
                bottom: 20,
